...
class req_handler(object):
    ...
    def handle_proxy_init(worker, addr):
        ...
        for atbl in msg.g_list_tbl:
            ...
            for row in result:
                ...
                worker.dbcon.call_proc(msg.g_proc_add_snd_req, ('dns',
                 msg.g_dict_type[atbl],
                 row[1],
                 row[0],
                 0,
                 msg.g_opt_add))
                msgobj.append({'opt': msg.g_opt_add,
                 'domain': row[0],
                 'view': row[1],
                 'type': msg.g_dict_type[atbl],
        ...
        if result:
            for row in result:
                print row
                worker.dbcon.call_proc(msg.g_proc_add_snd_req, ('view',
                 0,
                 row[0],
                 row[1],
                 0,
    ...
    def handle_proxy_init_reply(worker, answ, addr):
        ...
        print (str_class,
         ...
         answ['opt'])
        if True:
            worker.dbcon.call_proc(msg.g_proc_update_snd_req, (str_class,
             answ['type'],
             answ['viewid'],
             answ['data'],
             state_set,
    ...
    def handle_inner_chk_task(http_th, worker, _type):
        ...
        print ('test pre ioopt--> utf8:', repr(payload['ioopt']))
        test_de = json.loads(payload_encode[5:])['ioopt']
        print ('test enc and dec ioopt:',
         test_de.encode('UTF-8'),
         ', utf8:',
         repr(test_de.encode('UTF-8')),
         ', unicode:',
         repr(test_de))
    ...
    def handle_inner_chk_task_reply(worker, data):
        ...
        for task_id in dic_result:
            ...
            if not found:
                ptr_tasks[task_id] = {'ret': 1,
                 'result': 'task id ' + task_id + ' failed',
                 'error': 'table error'}
                continue
            try:
            ...
            if not db_ret:
                ptr_tasks[task_id] = {'ret': 1,
                 ...
                 'error': 'db error'}
                continue
            ptr_tasks[task_id] = {'ret': 0,
             'result': 'task id ' + task_id + ' succ',
             'error': ''}
            if go_on:
                try:
    ...
    def donotify(self, worker, msgobj, opt, data, odata, real_tbl):
        ...
        for case in switch(opt):
            if case('add'):
                if odata and len(odata) > 0 and len(odata[0]) >= 4:
                    ...
                    msgobj.append({'opt': http_opt_str2int[ropt],
                     'domain': odata0[0],
                     'view': odata0[1],
                     'type': msg.g_dict_type[odata0[2]],
                     'pkt_head': msg.g_pack_head_init_dns})
                    req_handler.notify_proxy(worker, msgobj, worker.proxy_addr.keys()[0], False)
                msgobj.append({'opt': http_opt_str2int['add'],
                 'domain': data['name'].lstrip('@.'),
                 'view': int(data['viewid']),
                 'type': msg.g_dict_type[real_tbl],
                 'pkt_head': msg.g_pack_head_init_dns})
                break
            if case('set'):
                if odata and len(odata) > 0 and len(odata[0]) >= 4:
            ...
            if case('del'):
                ...
                break
            if case():
                print ('opt:', opt, ' has not been implemented!')
...
class req_handler_record_a(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:a_record into database')
        return req_handler_impl.add(self, worker, data, ali_tbl)

    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:a_record into database')
        return req_handler_impl.set(self, worker, data, ali_tbl)

    def delete(self, worker, data, ali_tbl):
        print ('deleting name:a_record rid:', data['rid'], ' from database')
...
class req_handler_record_aaaa(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:aaaa_record into database')
        data[ali_tbl] = urllib.unquote(str(data[ali_tbl]))
        print ('decode aaaa:', data[ali_tbl])
        return req_handler_impl.add(self, worker, data, ali_tbl)

    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:aaaa_record into database')
        data[ali_tbl] = urllib.unquote(str(data[ali_tbl]))
        print ('decode aaaa:', data[ali_tbl])
        return req_handler_impl.set(self, worker, data, ali_tbl)

    def delete(self, worker, data, ali_tbl):
        print ('deleting name:aaaa_record rid:', data['rid'], ' from database')
        return req_handler_impl.delete(self, worker, data, ali_tbl)
...
class req_handler_record_cname(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:cname_record into database')
        return req_handler_impl.add(self, worker, data, ali_tbl)

    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:cname_record into database')
        return req_handler_impl.set(self, worker, data, ali_tbl)

    def delete(self, worker, data, ali_tbl):
        print ('deleting name:cname_record rid:', data['rid'], ' from database')
        return req_handler_impl.delete(self, worker, data, ali_tbl)
...
class req_handler_record_ns(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:ns_record into database')
        return req_handler_impl.add(self, worker, data, ali_tbl)

    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:ns_record into database')
        return req_handler_impl.set(self, worker, data, ali_tbl)

    def delete(self, worker, data, ali_tbl):
        print ('deleting name:ns_record rid:', data['rid'], ' from database')
        return req_handler_impl.delete(self, worker, data, ali_tbl)
...
class req_handler_record_txt(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:txt_record into database')
        data[ali_tbl] = urllib.unquote(str(data[ali_tbl]))
        print ('decode TXT:', data[ali_tbl])
        return req_handler_impl.add(self, worker, data, ali_tbl)

    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:txt_record into database')
        data[ali_tbl] = urllib.unquote(str(data[ali_tbl]))
        print ('decode TXT:', data[ali_tbl])
        return req_handler_impl.set(self, worker, data, ali_tbl)

    def delete(self, worker, data, ali_tbl):
        print ('deleting name:txt_record rid:', data['rid'], ' from database')
        return req_handler_impl.delete(self, worker, data, ali_tbl)
...
class req_handler_record_mx(req_handler_impl):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding name:', data['name'], ' table:mx_record into database')
        n_enable = 1
        if data.has_key('enable'):
            n_enable = int(data['enable'])
        add_ret = worker.dbcon.call_proc(msg.g_proc_add_mx_record, (data['name'].lstrip('@.'),
         data['main'],
         data['viewid'],
         data['ttl'],
         data['level'],
    ...
    def set(self, worker, data, ali_tbl):
        print ('updating name:', data['name'], ' table:mx_record into database')
        n_enable = 1
        if data.has_key('enable'):
            n_enable = int(data['enable'])
        update_ret = worker.dbcon.call_proc(msg.g_proc_add_mx_record, (data['name'].lstrip('@.'),
         data['main'],
         data['viewid'],
         data['ttl'],
         data['level'],
    ...
    def delete(self, worker, data, ali_tbl):
        print ('deleting name:mx_record rid:', data['rid'], ' from database')
        return req_handler_impl.delete(self, worker, data, ali_tbl)
...
class req_handler_record_domain_ns(req_hdl_abstract):
    ...
    def add(self, worker, data, ali_tbl):
        print ('adding domain_ns:',
         data['main'],
         '-->',
         data['domain_ns'],
         ' into database')
        add_ret = worker.dbcon.query(msg.g_sql_add_a_domain_ns % (data['main'],
         int(data['ttl']),
         data['domain_ns'],
         data['rid'],
         data['main'],
         int(data['ttl']),
         data['domain_ns']))
        print ('add return ', add_ret)
        return (add_ret, False, None)

    def set(self, worker, data, ali_tbl):
        print ('updating domain_ns:',
         data['main'],
         '-->',
         data['domain_ns'],
         ' into database')
...
class req_handler_domain(req_handler_impl):
    ...
    def delete(self, worker, data, ali_tbl):
        if False:
            data['enable'] = 0
            return self.set(worker, data, ali_tbl)
        print ('deleting domain:', data['name'], ' from database')
...
http_tbl_realname = {'A': 'a_record',
 ...
 'MX': 'mx_record',
 'domain_ns': 'domain_ns'}
http_opt_str2int = {'add': msg.g_opt_add,
 'del': msg.g_opt_del}
