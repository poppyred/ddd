    ...
    def handler(self, data):
        ...
        try:
            ...
            if data['class'] == msg.g_class_proxy_register:
                print (data['class'] + '... expect[%d]' % (msg.g_init_resp_expect,))
                self.proxy_addr[data['inner_addr'][0]] = [data['inner_addr'], self.proxy_health]
            ...
            if data['class'] == msg.g_class_inner_chk_init_ok:
                ...
                if ok_cnt:
                    print (data['class'] + '... expect[' + str(msg.g_init_resp_expect) + '][' + str(ok_cnt) + ']')
                else:
                    print (data['class'] + '... expect[' + str(msg.g_init_resp_expect) + '][NULL]')
