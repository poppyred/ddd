...
static DIR *opendir(const char *dirname)
{
    ...
    dirp = (DIR*) h_malloc (sizeof (struct DIR));
    if (dirp != NULL) {
        int error;
...
int h_dirutils_listdir(const char *dir, void *userdata,
        int entry_callback(int isdir, const char *entry_name, void *userdata))
{
    DIR *dip;
    struct dirent *dit;
    int ret;
    ...
    while ((dit = readdir(dip)) != NULL) {
        ...
        sprintf_n(entry_path, PATH_MAX, "%s/%s", dir, dit->d_name);

#ifdef _MSC_VER
        isdir = (dit->d_type == DT_DIR);
#else
        {
            struct stat sbuf;

            if (lstat(entry_path, &sbuf) != 0) {
                continue;
            }
        ...
        if ((ret = entry_callback(isdir, entry_path, userdata))) {
            closedir(dip);
            return ret;
        }
    }
        ...
        dir_entry_func_t entry_callback)
{
   ...
   else 
   {
       ...
       while (n--) 
       {
           ...
           sprintf_n(entry_path, PATH_MAX, "%s/%s", dir, namelist[n]->d_name);
...
#endif
           ret = entry_callback(isdir, entry_path, userdata);
           if( ret)
           {
               stop = 1;
           }
...
static int do_rmdir(int isdir, const char *entry_name, void *userdata)
{
    if (isdir) {
        int res;
        if ((res = h_dirutils_listdir(entry_name, userdata, do_rmdir)))
            return res;

#ifdef _MSC_VER
        /* vc don't have rmdir */
        return remove(entry_name);
#else
        return rmdir(entry_name);
#endif
