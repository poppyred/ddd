...
class DomainAction extends BaseAction {
	...
	public function detail(){
		if(!empty($_GET['d'])){
			...
			$record = M("domain")->query('select * from client_domain a left join domain b on b.id=a.domain_id where a.zone_id=%d and client_id=%d order by a.id desc',$zone["id"],$_SESSION['id']);
...
//			print(123);
			...
			//echo '{"result":"success","id":'.$ret["id"].'}'; //,"session":"'.$_SESSION['user'].'"
		}	
	}
	//域名记录
		...
		if(!empty($_POST['zone'])){
			...
			if($vo){
				if($vo['client_id']==$_SESSION['id']){
					...
					$this->ajaxReturn('域名被其他账户添加。','error',0);	
				}
		   	}else{
		...
		if(!empty($_POST['zone'])){
			...
			if($rslt["ret"] == 0){
				...
				foreach($record as $n => $value){
					...
					$ds = array("name"=>$dt['host'].".".$dt['domain'], "main"=>$data['domain'], "rid"=>$d_id, "domain_ns"=>$dt['val'], "level"=>$dt['mx'], "ttl"=>(int)($dt['ttl'])*60, "viewid"=>$dt['view']);
	...
	public function deleteZone(){
		if(!empty($_POST['domain'])){
			...
			for($i=0;$i<count($arr);$i++){
				...
				if($rslt["ret"] == 0){
					...
					for($j=0;$j<count($temp2);$j++){
						...
						$val = array("rid"=>$data['id'], $data['type']=>$data['val']);
						$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
			...
			for($i=0;$i<count($arr);$i++){
				$temp = $zone->where('domain="'.$arr[$i].'"')->find();
				$list = $zone_name->where('zone_id='.$temp['id'])->find();
...
//			print_r($_POST['domain']);
	...
	public function sortDomain(){
		if(!empty($_POST['zone'])){
			...
			$vo = $zone->where('domain=\''.$_POST['zone'].'\'')->find();
			$domain = M('domain');
			$record = $domain->query('select * from client_domain a left join domain b on b.id=a.domain_id where a.zone_id=%d and client_id=%d %s',$vo["id"],$_SESSION['id'], $_POST['ord']);
			//$record = $domain->query('select d.*,z.domain,v.name from client_domain cd left join domain d on d.id=cd.domain_id left join zone z on cd.zone_id=z.id LEFT JOIN view v ON d.route = v.id where z.domain=\''.$_POST['zone'].'\' '.$_POST['ord']);
			//print_r($domain->getLastSQL());
	...
	public function addDomain(){
		...
		$entity = $domain->query("select * from domain d left join client_domain cd on d.id=cd.domain_id where host='".$_POST['host']."' and type='".$_POST['type']."' and view=".$_POST['view']." and val='".$_POST['val']."' and mx=".$_POST['mx']." and cd.client_id=".$_SESSION['id']);
		...
		if($is_ok){
			$val = array("name"=>$data['host'].".".$_POST['zone'], "main"=>$_POST['zone'], "rid"=>$is_ok, $data['type']=>rawurlencode($data['val']), "level"=>$data['mx'], "ttl"=>(int)($data['ttl'])*60, "viewid"=>$data['view']);
			$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
	...
	public function updateDomain(){
		...
		$entity = $domain->query("select * from domain d left join client_domain cd on d.id=cd.domain_id where host='".$_POST['host']."' and type='".$_POST['type']."' and view=".$_POST['view']." and val='".$_POST['val']."' and mx=".$_POST['mx']." and cd.client_id=".$_SESSION['id']." and d.id not in (".$_POST['id'].") ");
		...
		$val = array("name"=>$data['host'].".".$_POST['zone'], "main"=>$_POST['zone'], "rid"=>$_POST['id'], $data['type']=>rawurlencode($data['val']), "level"=>$data['mx'], "ttl"=>(int)($data['ttl'])*60, "viewid"=>$data['view']);
		$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
	...
	public function deleteDomain(){
		if(!empty($_POST['id'])){
			...
			for($i=0;$i<count($arr);$i++){
				...
				//$val = array("name"=>$data['host'].".".$_POST['zone'], "main"=>$_POST['zone'], $data['type']=>$data['val'], "ttl"=>$data['ttl'], "viewid"=>$data['view']);
				$val = array("rid"=>$arr[$i], $data['type']=>$data['val']);
				$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
				$param = array("type"=>"record", "opt"=>"del", "data"=>$val,"user"=>$user);
				$ret = http_post(C('INTERFACE_URL')."/dnspro/dnsbroker/", $param);
				$rslt = json_decode($ret["content"],true);
	...
	public function domain_biem_update($id,$zone,$enable){
		...
		if(!empty($list)){
			foreach($list as $value){
				$val = array("name"=>$data['host'].".".$zone, "main"=>$zone, "rid"=>$id, $data['type']=>rawurlencode($data['val']), "level"=>$data['mx'], "ttl"=>(int)($data['ttl'])*60, "viewid"=>$data['view'],"enable"=>$enable);
				$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
				$param = array("type"=>"record", "opt"=>"set", "data"=>$val,"user"=>$user);
				$ret = http_post(C('INTERFACE_URL')."/dnspro/dnsbroker/", $param);
				$rslt = json_decode($ret["content"],true);
	...
	public function stopDns(){
		if(!empty($_POST['id'])){
			...
			for($i=0;$i<count($arr);$i++){				
				$data = $domain->where('id='.$arr[$i])->find();
				$val = array("name"=>$data['host'].".".$_POST['zone'], "main"=>$_POST['zone'], "rid"=>$arr[$i], $data['type']=>rawurlencode($data['val']), "level"=>$data['mx'], "ttl"=>(int)($data['ttl'])*60, "viewid"=>$data['view'],"enable"=>0);
				$user = array("cid"=>$_SESSION['id'], "level"=>0, "info"=>"");
				$param = array("type"=>"record", "opt"=>"set", "data"=>$val,"user"=>$user);
				$ret = http_post(C('INTERFACE_URL')."/dnspro/dnsbroker/", $param);
				$rslt = json_decode($ret["content"],true);
